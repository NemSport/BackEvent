...
test("almindeligt salg bliver behandlet med godkendt mapping og lagerkilde", () => {
  const [decision] = buildSyncDecisions([line()], [approvedMapping], [product], locations);
...
test("0-prislinje kan stadig behandles når mapping er godkendt", () => {
  const [decision] = buildSyncDecisions([line({ revenue: 0 })], [approvedMapping], [product], locations);
  assert.equal(decision.status, "processed");
...
test("manglende mapping ignoreres uden lagertræk", () => {
  const [decision] = buildSyncDecisions([line()], [], [product], locations);
...
test("pant og retur behandles ikke som normalt vareforbrug", () => {
  ...
  const [decision] = buildSyncDecisions([line({ ...classification, onlineposProductName: "RETUR Krus", onlineposProductGroupName: "Pant" })], [approvedMapping], [product], locations);
  assert.equal(decision.status, "ignored");
});
