...
export async function buildReceiptControlWorkbook(items: ExportControl[]) {
  ...
  for (const item of items) {
    ...
    sheet.addRow({
      ...
      depositReturnValue: item.depositReturnValue,
  ...
  summary.addRow({ label: "Samlet negativ total", value: items.filter((item) => item.finalTotal < 0).reduce((sum, item) => sum + Math.abs(item.finalTotal), 0) });
  summary.addRow({ label: "Samlet pant retur", value: items.reduce((sum, item) => sum + item.depositReturnValue, 0) });
