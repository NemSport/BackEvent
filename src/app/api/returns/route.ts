...
export async function GET(request: Request) {
  ...
  const receiptControlsResult = auth.canControl && (control === "open" || control === "history")
    ? await auth.supabase
      ...
      .select("id,receipt_number,onlinepos_transaction_id,classification,control_types,deposit_return_quantity,deposit_breakdown,purchase_value,deposit_return_value,final_total,purchase_value_including_vat,deposit_return_value_including_vat,final_total_including_vat,amount_source_details,source,replay_run_id,status,handled_at,handled_by_name,internal_note,created_at,updated_at,transaction_datetime,location_id,location_name,cash_register_id,cash_register_name,location_mapping_status")
